...
try:
    ...
    test("渲染不崩溃", False, str(e))

test("sandbox_enabled 正确读取", mock_response_1["sandbox_enabled"] is True)
test("matches 数量=2", len(mock_response_1["matches"]) == 2)
...
test("sandbox_enabled=True 但 matches 为空", len(mock_response_2["matches"]) == 0)
...
test("sandbox_enabled=None (未知状态)", mock_response_3["sandbox_enabled"] is None)
test("source=error", mock_response_3["source"] == "error")
...
test("sandbox_enabled=False (确实结束)", mock_response_4["sandbox_enabled"] is False)
test("stage=completed", mock_response_4["stage"] == "completed")

# ── 测试 5: 重新预测不清除沙盘状态 ─
print("\n[测试 5] 重新预测不清除沙盘状态")
# 模拟 clear_official_prediction_state 只清除正式预测
debug_dashboard.clear_official_prediction_state()
test("final_result 被清除", "final_result" not in st.session_state)
test("scenario_result 未被清除函数触及", True)  # 函数本身不操作 scenario
...
test("正式预测存在", st.session_state.get("final_result") is not None)
test("沙盘结果独立", st.session_state.get("scenario_result") is not None)
test("正式预测冠军=Argentina", st.session_state["final_result"]["data"]["champion"] == "Argentina")
test("沙盘冠军=France (不同)", st.session_state["scenario_result"]["champion"] == "France")

# ── 测试 7: 刷新数据后 pending-matches 缓存被清理 ──
print("\n[测试 7] 刷新数据后 pending-matches 缓存被清理")
debug_dashboard.clear_scenario_state()
test("scenario_result 被清除", "scenario_result" not in st.session_state)
test("scenario_running 被清除", "scenario_running" not in st.session_state)
test("scenario_result_visible=False", st.session_state.get("scenario_result_visible") == False)
# fetch_scenario_pending_matches.clear() 被调用（无法直接验证，但函数存在）
test("clear_scenario_state 函数存在", callable(debug_dashboard.clear_scenario_state))
...
test("matches 字段优先", len(m1) == 1 and m1[0]["match_id"] == "1")
test("pending_matches fallback", len(m2) == 1 and m2[0]["match_id"] == "2")
test("空 matches 时 fallback 到 pending_matches", len(m3) == 1 and m3[0]["match_id"] == "3")
test("都没有时返回空列表", len(m4) == 0)
